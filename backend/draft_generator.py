...
class HFSpaceFullGenerator:
    ...
    def generate(self, context: ResponseContextPayload) -> str:
        ...
        payload = {
            ...
            # [G-REGEN-01] Piggyback regen_attempt inside rule_signal so Modal/HF Space
            # can reduce ADP-A temperature per regen without a new positional parameter
            # (which would break warm containers during rolling deploys).
            # rule_signal is always a dict (never None) so _regen_attempt reaches the
            # inference endpoint even on the first attempt (regen_attempt=0).
            # Note: _mode was removed 2026-05-29 — it gated _strip_questions() which is
            # now retired following ADP-C weight-level fix (Improvement 2).
            "rule_signal":        {
                **(rule_signal or {}),
