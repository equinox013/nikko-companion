...
from schemas.acp_schemas import DistressLevel, OperationalMode, ResponseContextPayload
...
def build_adp_c_system(context: ResponseContextPayload) -> str:
    ...
    base = base.replace(
        ...
        "prohibited in COMFORT Mode. "
        "PERMITTED EXCEPTION: exactly one soft open-ended continuation question is "
        "allowed — forms like 'want to tell me more?', 'what\\'s been going on?', "
        "'how long has this been weighing on you?', 'would you like to share more?'. "
        "These are presence-keeping questions that invite the user to continue, NOT "
        "strategy suggestions. Do NOT mark them as violations. "
        "At MODERATE or HIGH distress no question is required at all, but if present "
        "a single soft continuation question is still APPROVE, not REGENERATE. "
        "REQ-000-065. "
        "reason: one sentence explanation. "
        "Output ONLY the JSON object."
    )
