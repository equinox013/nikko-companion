...
> **Status:** Phases 1–4 signed off. Phase 7 infra live (2026-05-14). Phase 5 pre-gated (frontend UX complete; backend integration wired; spec reconciliation audit done). Phase 6 evaluation is next gate.
...
**Phase 4 progress (as of 2026-05-14) — dual-model architecture (Director-approved 2026-05-14):**

> **Architecture switch:** Mistral-7B-Instruct-v0.3 was infeasible on RTX 3070 8 GB VRAM (14 GB bf16; training exceeded 14h with no convergence). All Mistral notebooks and finetuning artifacts archived to `notebooks/mistral-7b/` and `finetuning/mistral-7b/`. New model stack: **Phi-3.5-mini-instruct** (ADP-A) + **Gemma-2-2b-it** (ADP-B/C). Steps 11–17 regenerated. See SPEC-400 §3.1 for full rationale.

| Step | Artifact | Model | Status |
|------|----------|-------|--------|
| 11 | ADP-C data generation (`notebooks/step11_adp_c_data_generation.ipynb`) | Gemma-2-2b-it | ⏳ Ready to run (regenerated 2026-05-14) |
| 12 | ADP-C QLoRA training (`notebooks/step12_adp_c_training.ipynb`) | Gemma-2-2b-it | ⏳ Ready after Step 11 |
| 13 | ADP-A dataset preparation (`notebooks/step13_adp_a_data_preparation.ipynb`) | — (data prep) | ⏳ Ready after Step 12 (requires ADP-C v1 oracle) |
| 14 | ADP-A QLoRA training (`notebooks/step14_adp_a_training.ipynb`) | Phi-3.5-mini-instruct | ⏳ Ready after Step 13. `weight_decay=0.01`, `lr=1e-4`, `trust_remote_code=True`. |
| 15 | ADP-B data generation (`notebooks/step15_adp_b_data_generation.ipynb`) | Gemma-2-2b-it | ⏳ Ready (can run parallel to Steps 13–14) |
| 16 | ADP-B QLoRA training (`notebooks/step16_adp_b_training.ipynb`) | Gemma-2-2b-it | ⏳ Ready after Step 15 |
| 17 | ADP-C v2 retraining (`notebooks/step17_adp_c_retraining.ipynb`) | Gemma-2-2b-it | ⏳ Ready after Steps 14+16 |
| 18 | ADP-A v2 data preparation (`notebooks/step18_adp_a_v2_data_preparation.ipynb`) | — (data prep) | ⏳ Ready after Step 17. Generous caps: Amod 3 000, ESConv 3 000, MentalChat 2 500, AnnoMI 1 500, EmpDial 1 500. ADP-C v2 oracle. |
| 19 | ADP-A v2 QLoRA training (`notebooks/step19_adp_a_v2_training.ipynb`) | Phi-3.5-mini-instruct | ⏳ Ready after Step 18. `NUM_EPOCHS=15`, `MAX_SEQ_LENGTH=512`, `eval_steps=20`, `save_steps=40`, `load_best_model_at_end=True`, `weight_decay=0.01`, `lr=1e-4`. |

**Stable dependency stack (pinned in `environment.yml`):**
`transformers==4.46.3` · `accelerate==1.1.0` · `peft==0.13.2` · `trl==0.11.4` · `datasets==3.1.0` · `sentencepiece>=0.2.0` · `protobuf>=3.20.0`

> `bitsandbytes` **removed** from production stack. ZeroGPU only allocates GPU inside `@spaces.GPU`; bitsandbytes checks for CUDA at import time and crashes before any GPU context is established. Phi-3.5-mini and Gemma-2-2b-it both load cleanly in native bf16 on A10G 24 GB without quantization.
...
- `max_memory={0: "5000MiB", "cpu": "16GiB"}` — Phi-3.5-mini and Gemma-2 both fit in 8 GB RTX 3070 VRAM for training (no quantization required at 2B–4B scale with LoRA).
...
| Phase 7 (infra) — Deployment scaffolding | **✅ SIGNED OFF 2026-05-14** | Director sign-off | GitHub Pages (GH Actions), Fly.io backend (nikko-companion.onrender.com), HF Spaces ZeroGPU (ADP-A/B/C). Stack confirmed live: `space_ok=true`. See §8c delivery notes below. |
...
### Phase 7 infra — Delivery notes (signed off 2026-05-14)

**HF Space (`hf_space/app.py`) — critical changes:**

| Change | Detail |
|--------|--------|
| Model architecture | **Dual-model stack (Director-approved 2026-05-14):** Phi-3.5-mini-instruct (ADP-A) + Gemma-2-2b-it (ADP-B/C shared base). Mistral-7B-Instruct-v0.3 was infeasible on RTX 3070 8 GB VRAM; archived to `hf_space/mistral-7b/`, `notebooks/mistral-7b/`, `finetuning/mistral-7b/`. Total A10G VRAM budget: ~15 GB (fits 24 GB with headroom). |
| Model precision | bf16 (native). `BitsAndBytesConfig` / NF4 quantization dropped entirely — bitsandbytes checks for CUDA at import time; ZeroGPU only allocates GPU inside `@spaces.GPU`, causing import-time crash. Both Phi-3.5-mini and Gemma-2-2b-it load cleanly in bf16 within A10G VRAM budget. |
| `bitsandbytes` removed | Removed from `hf_space/requirements.txt`. `sentencepiece>=0.2.0`, `protobuf>=3.20.0` retained (required for both tokenizers). `trust_remote_code=True` required for Phi-3.5-mini. |
| `/pipeline` endpoint | **ADP-B → ADP-A → ADP-C now run in a single `@spaces.GPU(duration=300)` session.** Previously 3 separate `/infer` calls each incurring an 80–110s CPU→VRAM transfer; consolidation eliminates 2 transfers. Warm-turn latency: ~20–40s. Cold start: ~90–120s. The endpoint accepts `{messages, system, safety_system, eval_system, token}` and returns `{text, is_crisis, flags, verdict, regen, elapsed}`. |
| `/infer` endpoint | Retained for compatibility; not used by the backend in Phase 7+. |
| Launch mode | `uvicorn.run(app, host='0.0.0.0', port=7860)` in `__main__`, where `app = gr.mount_gradio_app(fastapi_app, _demo, path='/ui')`. Previous `_demo.launch()` in `__main__` bypassed FastAPI routes — fixed. |

**Backend (`backend/main.py`) — changes:**

| Change | Detail |
|--------|--------|
| `PIPELINE_TIMEOUT_S = 360` | Replaces `INFER_TIMEOUT_S = 90`. 360s provides headroom for cold start (~120s) + ADP-C regen pass (~30s) + overhead. |
| `_call_pipeline()` | POSTs all three system prompts to `/pipeline` in a single request. Replaces three separate `_infer()` calls. |
| `SSEChunk.trace` | New optional `trace: dict \| None` field on the SSE chunk model. The final substantive chunk carries pipeline metadata (`adp_b`, `adp_a`, `adp_c` results, `elapsed`, `regen`, `verdict`) for the frontend debug panel. |

**Frontend (`web/`) — changes:**

| Change | Detail |
|--------|--------|
| `ThinkingBubble` component | Staged waiting indicator with timed label progression (0–6s, 6–14s, 14–24s) then affirmation cycling every 5s beyond 24s. Replaces static three-dot spinner. Manages user expectation during 30–120s pipeline wait. |
| `agent-debug.jsx` rewrite | Debug overlay now shows real ADP-B / ADP-A / ADP-C adapter results from live pipeline trace data (via `NikkoAgentLog` pub/sub store on `window.__nikkoAgentLog`). Removed SPEC-700 header and simulated trace. Fallback trace (`buildAgentTrace`) uses keyword classification for offline/error states. |
| `NikkoAgentLog` | Shared trace store; `NikkoAgentLog.add({id, userText, ...trace, liveData: true})` called in `chat.jsx` SSE handler when `data.trace` is present on a chunk. |
| Compiled outputs | `web/chat.js` and `web/agent-debug.js` regenerated via `esbuild@0.25.3`. |
...
- **Loading screen:** Frontend polls `GET /health` on Fly.io from app load; interactive loading screen shown until `200` (see FRONTEND_INTEGRATION_SPEC §12).
